...
if [[ $SOURCE == "umls" ]]; then
  ...
  DOWNLOAD_URLS=($SNOMED_ICD10_URL $SNOMED_CORE_URL $SNOMED_ORPHANET_URL $SNOMED_MedDRA_URL $SNOMED_ORPHANET_URL $SNOMED_MAPSET_URL $SNOMED_ICD9_URL $ICD9_URL $UMLS_URL $ICD11_URL $ICD11_MAPPING_URL)
else 
  echo "Wrong source"
fi
