...
if [[ $SOURCE == "umls" ]]; then
  ...
  DOWNLOAD_URLS=($SNOMED_ICD10_URL $SNOMED_CORE_URL $SNOMED_ORPHANET_URL $SNOMED_MedDRA_URL $SNOMED_ORPHANET_URL $SNOMED_MAPSET_URL $SNOMED_ICD9_URL $ICD9_URL $UMLS_URL $ICD11_URL $ICD11_MAPPING_URL $ICD10_URL $NCIT_URL $ICDO_URL $MONDO_URL $ORPHANET_URL $ORPHANET_SUBCLASS_URL $MESH_DES_URL $MESH_SCR_URL)
else
  echo "Wrong source"
fi
